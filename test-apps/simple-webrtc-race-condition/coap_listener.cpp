...
CoapListenerPtr CoapListener::create(nabto::NabtoDevicePtr device, NabtoDeviceCoapMethod method, std::vector<std::string> path, nabto::EventQueuePtr queue)
{
...
bool CoapListener::start(NabtoDeviceCoapMethod method, std::vector<std::string> path)
{
    // Convert path to a temporary NULL terminated list of c_strings
    std::vector<const char *> pathSegments;
    for(std::string& p : path) {
        pathSegments.push_back(p.c_str());
    }
    pathSegments.push_back(NULL);

    if (listener_ == NULL ||
        future_ == NULL ||
        nabto_device_coap_init_listener(device_.get(), listener_, method, pathSegments.data()) != NABTO_DEVICE_EC_OK)
    {
