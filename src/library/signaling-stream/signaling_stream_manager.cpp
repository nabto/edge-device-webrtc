...
bool SignalingStreamManager::connectionAddMediaTracks(const std::string& id, const std::vector<MediaTrackPtr>& tracks)
{
    SignalingStreamPtr stream = findStream(id);
    if (stream) {
        return stream->createTracks(tracks);
    }
    return false;
}
